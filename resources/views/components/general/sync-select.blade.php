...
class="relative w-full" @click.away="active=false">
    {{@$slot}}
    <button @click="active=true;$nextTick(()=>{ $refs.focusInput.focus() })" class="w-full px-3 py-1 text-xs font-semibold border">
        <span x-show="!value" class="text-gray-500">Please select</span>
        <template x-for="(item, index) in list" :key="item.value">
            <span x-show="item.value===value" x-text="item.text"></span></template>
    </button>
    <template  x-if="active" x-cloak="active">
        <ul class="absolute z-20 w-full bg-white border border-blue-500 shadow">
            <li><input @focus="active=true" x-ref="focusInput" type="text" class="w-full px-4 py-2 text-xs border border-gray-500" x-model="search"></li>
